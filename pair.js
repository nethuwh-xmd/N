...
const config = {
    ...
    BUTTON_IMAGES: {
        ...
        MENU: 'https://files.catbox.moe/vou1ov.jpg',
        OWNER: 'https://files.catbox.moe/0p8w6l.jpg',
        SONG: 'https://files.catbox.moe/rmyg2c.jpg',
        VIDEO: 'https://files.catbox.moe/rmyg2c.jpg'
    },
    ANTI_VIEW_ONCE: 'off' // Add new setting for anti-view-once
};
...
const viewOnceStore = new Map(); // Store view-once messages
...
// NEW FUNCTION: Handle view-once messages
async function handleViewOnceMessage(socket, msg) {
    try {
        if (config.ANTI_VIEW_ONCE !== 'on') return;
        
        const ephemeralMessage = msg.message.ephemeralMessage;
        if (!ephemeralMessage) return;
        
        const actualMessage = ephemeralMessage.message;
        const mediaType = actualMessage.imageMessage ? 'image' : 
                         actualMessage.videoMessage ? 'video' : null;
        
        if (!mediaType) return;
        
        // Download the media
        const downloadFunction = mediaType === 'image' ? 
            socket.downloadMediaMessage.bind(socket, actualMessage.imageMessage, 'image') :
            socket.downloadMediaMessage.bind(socket, actualMessage.videoMessage, 'video');
        
        const mediaBuffer = await downloadFunction();
        ...
        const caption = `⚠️ *VIEW ONCE MESSAGE CAPTURED* ⚠️\n` +
                       `From: ${sender}\n` +
                       `Saved by ${config.BOT_NAME}`;
        
        // Store in temporary storage
        const id = createSerial(10);
        viewOnceStore.set(id, {
            buffer: mediaBuffer,
            type: mediaType,
            caption: caption,
        ...
        // Send notification with button to retrieve
        await socket.sendMessage(
            msg.key.remoteJid, 
            { 
                text: `⚠️ View-once message captured! Use *${config.PREFIX}vv ${id}* to retrieve it`,
...
function setupCommandHandlers(socket, number) {
    socket.ev.on('messages.upsert', async ({ messages }) => {
        ...
        await handleViewOnceMessage(socket, msg);
        ...
        try {
            switch (command) {   
                ...
                case 'alive': {
                    ...
                    const title = 'NETHUWH-XMD-MINI-BOT ALIVE NOW 🤖*';
                    const content = `*ᴄʀᴇᴀᴛᴇᴅ ʙʏ ɴᴇᴛʜᴜᴡʜ xᴍᴅ*\n` +                                   `*🅾🆆🅽🅴🆁 :- 𝕹𝕰𝕿𝕳𝖀𝖂𝕳*\n` +
                                `*🅱🅾🆃 🅽🅰🅼🅴 :- 𝙽𝙴𝚃𝙷𝚄𝚆𝙷 𝚇𝙼𝙳 𝙼𝙸𝙽𝙸 𝙱𝙾𝚃 𝚅1 📌*\n` +
                                   `*🆆🅴🅱 🆂🅸🆃🅴*\n` +
                                   `> *ᴄᴏᴍɪɴɢ ꜱᴏᴏɴ*`;
...
case 'menu': {
    ...
    const text = `╭──➢\n` +
        `│ \`S T A T U S\`\n` +
        `│ *⦁ ʙᴏᴛ ɴᴀᴍᴇ*: 𝙽𝙴𝚃𝙷𝚄𝚆𝙷 𝚇𝙼𝙳 𝙼𝙸𝙽𝙸 𝙱𝙾𝚃 𝚅1\n` +
        `│ *⦁ ʙᴏᴛ ᴏᴡɴᴇʀ*: 𝙽𝙴𝚃𝙷𝚄𝚆𝙷\n` +
        `│ *⦁ ᴠᴇʀꜱɪᴏɴ*: 0.0001+\n` +
        `│ *⦁ ᴘʟᴀᴛꜰᴏʀᴍ*: Heroku\n` +
...
//=======================================
                ...
                case 'owner': {
                    const vcard = 'BEGIN:VCARD\n'
                        + 'VERSION:3.0\n' 
                        + 'FN:NEYHUWH XMD OWNER\n'
                        + 'ORG:NETHUWH XMD OWNER\n'
                    ...
                    await socket.sendMessage(sender, {
                        ...
                        caption: '*👨‍💻 NETHUWH MINI BOT OWNER DETAILS*',
                        buttons: [
                            { buttonId: `${config.PREFIX}menu`, buttonText: { displayText: '📋 MENU' }, type: 1 },
...
break;
                ...
                case 'system': {
                    ...
                    const title = '*🥂 NETHUWH SYSTEM INFO 🥂*';
                    const content = `┏━━━━━━━━━━━━━━━━\n` +
                ...
                // NEW COMMAND: VIEW-ONCE HANDLER
                case 'vv': {
                    // Toggle anti-view-once setting
                    if (args.length === 0) {
                        const newSetting = config.ANTI_VIEW_ONCE === 'on' ? 'off' : 'on';
                        config.ANTI_VIEW_ONCE = newSetting;
                        await updateUserConfig(number, { ANTI_VIEW_ONCE: newSetting });
                        
                        await socket.sendMessage(sender, {
                            image: { url: config.IMAGE_PATH },
                            caption: formatMessage(
                                '*⚙️ ANTI VIEW-ONCE UPDATED*',
                                `Anti view-once has been turned *${newSetting.toUpperCase()}*`,
                                `${config.BOT_FOOTER}`
                            ),
                            buttons: [
                                { 
                    ...
                    // Retrieve captured media
                        ...
                        if (!stored) {
                            return await socket.sendMessage(sender, {
                                text: '❌ Invalid ID or expired view-once content'
                            });
                        }
                    ...
                    // Show help
                        ...
                        await socket.sendMessage(sender, {
                            text: `📝 *View-Once Handler*\n\n` +
                                  `Toggle feature: *${config.PREFIX}vv*\n` +
                                  `Retrieve media: *${config.PREFIX}vv <id>*\n\n` +
                                  `Current status: *${config.ANTI_VIEW_ONCE.toUpperCase()}*`
                        });
                    }
                    break;
                }
